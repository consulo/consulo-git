...
class GitMergeOperation extends GitBranchOperation {
    ...
    protected void execute() {
        ...
        try (AccessToken token = DvcsUtil.workingTreeChangeStarted(myProject, getOperationName().get())) {
            while (hasMoreRepositories() && !fatalErrorHappened) {
                ...
                GitSimpleEventDetector alreadyUpToDateDetector =
                    new GitSimpleEventDetector(GitSimpleEventDetector.Event.ALREADY_UP_TO_DATE);
            ...
            if (fatalErrorHappened) {
                ...
                if (allConflictsResolved) {
                    if (alreadyUpToDateRepositories < getRepositories().size()) {
                        ...
                        notifySuccess(LocalizeValue.localizeTODO("Already up-to-date"));
                    }
                }
            }
    ...
    private boolean proposeSmartMergePerformAndNotify(
        @Nonnull GitRepository repository,
        @Nonnull GitMessageWithFilesDetector localChangesOverwrittenByMerge
    ) {
        Pair<List<GitRepository>, List<Change>> conflictingRepositoriesAndAffectedChanges =
            getConflictingRepositoriesAndAffectedChanges(repository, localChangesOverwrittenByMerge,
    ...
    private boolean doSmartMerge(@Nonnull Collection<GitRepository> repositories) {
        AtomicBoolean success = new AtomicBoolean();
        myPreservingProcess = new GitPreservingProcess(
            myProject,
            myGit,
    ...
    private LocalizeValue getCommonErrorTitle() {
        return LocalizeValue.localizeTODO("Couldn't merge " + myBranchToMerge);
    }
    ...
    @Nonnull
    private GitCompoundResult smartRollback(@Nonnull Collection<GitRepository> repositories) {
    ...
    public LocalizeValue getSuccessMessage() {
        return LocalizeValue.localizeTODO(String.format(
            "Merged <b><code>%s</code></b> to <b><code>%s</code></b>",
            myBranchToMerge,
    ...
    private class MyMergeConflictResolver extends GitMergeCommittingConflictResolver {
        public MyMergeConflictResolver() {
            super(
                GitMergeOperation.this.myProject,
                myGit,
                new GitMerger(GitMergeOperation.this.myProject),
