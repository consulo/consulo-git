...
public class GitImpl implements Git {
    ...
    @Nonnull
    @Override
    public Set<VirtualFile> untrackedFiles(
        @Nonnull Project project,
        @Nonnull VirtualFile root,
        @Nullable Collection<VirtualFile> files
    ) throws VcsException {
        Set<VirtualFile> untrackedFiles = new HashSet<>();
    ...
    public Collection<VirtualFile> untrackedFilesNoChunk(
        ...
        for (String relPath : output.split("\u0000")) {
            ...
            if (f == null) {
                ...
                LOG.info("VirtualFile for path [{}] is null", relPath);
            }
            else {
    ...
    @Nonnull
    @Override
    public GitCommandResult checkAttr(
        ...
        return runAll(ContainerUtil.map(
            listOfPaths,
            (Function<List<String>, Supplier<GitCommandResult>>) relativePaths -> () -> {
    ...
    @Nonnull
    @Override
    public GitCommandResult renameBranch(
        @Nonnull GitRepository repository,
    ...
    @Nonnull
    private static GitCommandResult reset(
        @Nonnull GitRepository repository,
    ...
    @Nonnull
    private GitCommandResult doPush(
        @Nonnull GitRepository repository,
        @Nonnull String remoteName,
        @Nullable String puttyKey,
        @Nonnull Collection<String> remoteUrls,
        @Nonnull String spec,
        boolean force,
        boolean updateTracking,
        boolean skipHook,
    ...
    @Override
    @Nonnull
    public GitCommandResult cherryPick(
        @Nonnull GitRepository repository,
        @Nonnull String hash,
        boolean autoCommit,
        @Nonnull GitLineHandlerListener... listeners
    ...
    @Nonnull
    @Override
    public GitCommandResult lsRemote(@Nonnull Project project, @Nonnull File workingDir, @Nonnull String url) {
        return doLsRemote(project, workingDir, url, singleton(url));
    }

    @Nonnull
    @Override
    public GitCommandResult lsRemote(
        @Nonnull Project project,
        @Nonnull VirtualFile workingDir,
        @Nonnull GitRemote remote,
        String... additionalParameters
    ) {
        return doLsRemote(project, VirtualFileUtil.virtualToIoFile(workingDir), remote.getName(), remote.getUrls(), additionalParameters);
    }

    @Nonnull
    @Override
    public GitCommandResult remotePrune(@Nonnull GitRepository repository, @Nonnull GitRemote remote) {
        return run(() -> {
            GitLineHandler h =
                new GitLineHandler(repository.getProject(), repository.getRoot(), GitCommand.REMOTE.writeLockingCommand());
            h.setStdoutSuppressed(false);
            h.addParameters("prune");
            h.addParameters(remote.getName());
            h.setUrls(remote.getUrls());
    ...
    @Nonnull
    @Override
    public GitCommandResult rebase(
        @Nonnull GitRepository repository,
    ...
    @Nonnull
    private GitCommandResult rebaseResume(
        @Nonnull GitRepository repository,
        @Nonnull GitRebaseResumeMode rebaseMode,
        @Nonnull GitLineHandlerListener[] listeners
    ) {
        Project project = repository.getProject();
        VirtualFile root = repository.getRoot();
    ...
    @Nonnull
    private GitCommandResult runWithEditor(
        @Nonnull Project project,
        @Nonnull VirtualFile root,
    ...
    @Nonnull
    protected GitInteractiveRebaseEditorHandler configureEditor(
        @Nonnull Project project,
        @Nonnull VirtualFile root,
        @Nonnull GitLineHandler handler,
        boolean commitListAware
    ...
    @Nonnull
    private static GitCommandResult doLsRemote(
        @Nonnull Project project,
        @Nonnull File workingDir,
        @Nonnull String remoteId,
        @Nonnull Collection<String> authenticationUrls,
        String... additionalParameters
    ) {
        return run(() -> {
    ...
    @Nonnull
    @Override
    public GitCommandResult runCommand(@Nonnull GitLineHandler handler) {
    ...
    private static boolean looksLikeError(@Nonnull String text) {
