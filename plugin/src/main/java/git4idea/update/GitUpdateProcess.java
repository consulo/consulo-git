...
public class GitUpdateProcess {
    ...
    private GitUpdateResult updateImpl(@Nonnull UpdateMethod updateMethod) {
            ...
            if (!problematicRoots.isEmpty()) {
                ...
                if (decision == GitRebaseOverMergeProblem.Decision.MERGE_INSTEAD) {
                    for (GitRepository repo : problematicRoots) {
                        ...
                        updaters.put(
                            repo,
                            new GitMergeUpdater(myProject, myGit, repo, branchAndTracked, myProgressIndicator, myUpdatedFiles)
                        );
                    }
                }
        ...
        Collection<VirtualFile> myRootsToSave = new ArrayList<>();
        ...
        SimpleReference<Boolean> incomplete = SimpleReference.create(false);
        SimpleReference<GitUpdateResult> compoundResult = SimpleReference.create();
            ...
            () -> {
                ...
                try {
                    ...
                    notifyImportantError(
                        ...
                        LocalizeValue.localizeTODO("Updating " + rootName + " failed with an error: " + e.getLocalizedMessage())
                    );
                }
            }
        ).execute(() -> {
        ...
        return ObjectUtil.notNull(compoundResult.get(), GitUpdateResult.ERROR);
    }
    ...
    private Map<GitRepository, GitUpdater> tryFastForwardMergeForRebaseUpdaters(@Nonnull Map<GitRepository, GitUpdater> updaters) {
        ...
        for (GitRepository repository : myRepositories) {
            ...
            if (updater instanceof GitRebaseUpdater rebaseUpdater
                ...
                && rebaseUpdater.fastForwardMerge()) {
                continue;
            }
    ...
    @Nonnull
    private Map<GitRepository, GitUpdater> defineUpdaters(
        @Nonnull UpdateMethod updateMethod,
        @Nonnull Map<VirtualFile, GitBranchPair> trackedBranches
    ) throws VcsException {
        Map<GitRepository, GitUpdater> updaters = new HashMap<>();
        ...
        for (GitRepository repository : myRepositories) {
            ...
            GitUpdater updater =
                GitUpdater.getUpdater(myProject, myGit, branchAndTracked, repository, myProgressIndicator, myUpdatedFiles, updateMethod);
    ...
    private Map<VirtualFile, GitBranchPair> checkTrackedBranchesConfiguration() {
        Map<VirtualFile, GitBranchPair> trackedBranches = new HashMap<>();
            ...
            if (branch == null) {
                ...
                notifyImportantError(
                    ...
                    LocalizeValue.localizeTODO(
                        "You are in 'detached HEAD' state, which means that you're not on any branch" + mention(repository) + "<br/>" +
            ...
            if (trackInfo == null) {
                ...
                if (myCheckForTrackedBranchExistence) {
                    notifyImportantError(
                        ...
                        LocalizeValue.localizeTODO("Can't Update"),
                        getNoTrackedBranchError(repository, branch.getName())
                    );
                    return null;
                }
            }
    ...
    @Nonnull
    static LocalizeValue getNoTrackedBranchError(@Nonnull GitRepository repository, @Nonnull String branchName) {
        String recommendedCommand = recommendSetupTrackingCommand(repository, branchName);
        return LocalizeValue.localizeTODO(
            "No tracked branch configured for branch " + code(branchName) + mention(repository) + " or the branch doesn't exist.<br/>" +
                "To make your branch track a remote branch call, for example,<br/>" + "<code>" + recommendedCommand + "</code>"
        );
    }
    ...
    private boolean areUnmergedFiles() {
        ...
        return !new GitMergeCommittingConflictResolver(
            ...
            myGit,
            myMerger,
            getRootsFromRepositories(myRepositories),
            params,
            false
        ).merge();
    }
