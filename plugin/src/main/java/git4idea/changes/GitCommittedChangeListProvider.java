...
public class GitCommittedChangeListProvider implements CommittedChangesProvider<CommittedChangeList, ChangeBrowserSettings> {
    private static final Logger LOG = LoggerFactory.getLogger(GitCommittedChangeListProvider.class);
    ...
    public void loadCommittedChanges(
        ...
        try {
            getCommittedChangesImpl(settings, location, maxCount, consumer::accept);
        }
        finally {
            consumer.finished();
        }
    ...
    public List<CommittedChangeList> getCommittedChanges(ChangeBrowserSettings settings, RepositoryLocation location, int maxCount)
        ...
        getCommittedChangesImpl(settings, location, maxCount, result::add);
    ...
    private void getCommittedChangesImpl(
        ...
        GitUtil.getLocalCommittedChanges(
            myProject,
            root,
            h -> {
    ...
    public Pair<CommittedChangeList, FilePath> getOneList(VirtualFile file, VcsRevisionNumber number) throws VcsException {
        ...
        List<GitHeavyCommit> gitCommits = GitHistoryUtils.commitsDetails(myProject, filePath, new SymbolicRefs(),
        ...
        CommittedChangeList commit = new GitCommittedChangeList(
            gitCommit.getDescription() + " (" + gitCommit.getShortHash().getString() + ")",
            gitCommit.getDescription(),
            gitCommit.getAuthor(),
            (GitRevisionNumber) number,
            new Date(gitCommit.getAuthorTime()),
        ...
        for (Change change : changes) {
            if (change.getAfterRevision() != null && FileUtil.filesEqual(
                filePath.getIOFile(),
