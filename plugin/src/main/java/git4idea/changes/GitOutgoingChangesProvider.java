...
public class GitOutgoingChangesProvider implements VcsOutgoingChangesProvider<CommittedChangeList> {
    private final static Logger LOG = LoggerFactory.getLogger(GitOutgoingChangesProvider.class);
    ...
    public Collection<Change> filterLocalChangesBasedOnLocalCommits(Collection<Change> localChanges, VirtualFile vcsRoot)
        ...
        try {
            ...
            LOG.info("Error while getting merge base", e);
        ...
        if (hashes.isEmpty()) {
            return Collections.emptyList(); // no local commits
        }
     ...
     */
    @Nullable
    private static GitRevisionNumber getMergeBase(
        @Nonnull Project project, @Nonnull VirtualFile root,
        @Nonnull GitBranch currentBranch, @Nonnull GitBranch branch
    ) throws VcsException {
